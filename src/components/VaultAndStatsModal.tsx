    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-[#f4f5f8] dark:bg-black text-slate-900 dark:text-slate-100 overflow-hidden animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between border-b border-black/5 dark:border-white/10 bg-white dark:bg-black px-4 py-3 shadow-sm pt-safe">
        ...
        <div className="text-center">
          <h2 className="text-base font-semibold text-slate-900 dark:text-white">Vault & Statistics</h2>
      ...
      {/* Main Stats Area */}
      <div className="flex-1 overflow-y-auto p-4 pb-32 md:pb-40 max-w-md mx-auto w-full space-y-4">
        {/* Streak Hero Card */}
        <div className="rounded-2xl border border-orange-200 dark:border-orange-900/40 bg-gradient-to-br from-amber-500 via-orange-500 to-rose-500 p-6 text-white shadow-xl shadow-orange-500/20 text-center space-y-2">
          ...
          <h3 className="text-4xl font-bold">{progress.streak} Day Streak 🔥</h3>
          <p className="text-xs font-bold text-orange-100">
            Keep practicing daily to maintain your fire and unlock bonus coins!
          </p>

          <div className="pt-2">
            <div className="flex justify-between text-xs font-semibold text-orange-100 mb-1">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-2xl border border-black/5 dark:border-white/10 bg-white dark:bg-black p-4 text-center shadow-sm">
            <span className="text-[10px] font-semibold uppercase text-slate-400">Total XP Earned</span>
            <div className="mt-1 flex items-center justify-center gap-1 text-2xl font-bold text-[#2563eb]">
          ...
          <div className="rounded-2xl border border-black/5 dark:border-white/10 bg-white dark:bg-black p-4 text-center shadow-sm">
            <span className="text-[10px] font-semibold uppercase text-slate-400">Coins Collected</span>
            <div className="mt-1 flex items-center justify-center gap-1 text-2xl font-bold text-amber-500">
          ...
          <div className="rounded-2xl border border-black/5 dark:border-white/10 bg-white dark:bg-black p-4 text-center shadow-sm">
            <span className="text-[10px] font-semibold uppercase text-slate-400">Gems Earned</span>
            <div className="mt-1 flex items-center justify-center gap-1 text-2xl font-bold text-purple-600 dark:text-purple-400">
          ...
          <div className="rounded-2xl border border-black/5 dark:border-white/10 bg-white dark:bg-black p-4 text-center shadow-sm">
            <span className="text-[10px] font-semibold uppercase text-slate-400">Hearts</span>
            <div className="mt-1 flex items-center justify-center gap-1 text-2xl font-bold text-rose-500">
        ...
        {/* Weak Words Section */}
        <div className="rounded-2xl border border-black/5 dark:border-white/10 bg-white dark:bg-black p-5 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="text-base font-semibold text-slate-900 dark:text-white">Weak Words Review</h4>
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-full bg-rose-500 hover:bg-rose-600 px-4 py-2 text-xs font-semibold text-white shadow-md shadow-rose-500/20 active:scale-95 cursor-pointer"
            >
          ...
          {progress.weakWords.length > 0 ? (
            <div className="space-y-2 pt-1">
              {progress.weakWords.slice(0, 4).map((w: VocabWord, idx: number) => (
                ...
                >
                  <span className="font-semibold text-slate-900 dark:text-white">{w.pt}</span>
                  <span className="font-semibold text-slate-500 dark:text-slate-400">"{w.en}"</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs font-bold text-[#2563eb] text-center py-2">
        ...
        <div className="flex items-center justify-between rounded-2xl border border-black/5 dark:border-white/10 bg-white dark:bg-black p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-slate-900 dark:text-white">Daily Study Reminders</h4>
              <p className="text-xs font-semibold text-slate-400">
          ...
          <button
            ...
            className={`h-7 w-12 rounded-full p-1 transition-colors cursor-pointer ${
              progress.remindersEnabled ? 'bg-[#2563eb]' : 'bg-slate-300 dark:bg-slate-700'
            }`}
          >
            <div
